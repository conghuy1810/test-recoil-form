...
import {
  ...
  getFieldArrayDataAndExtraInfoByRowId,
} from './atoms';
...
function FormValuesObserver() {
  ...
  useRecoilTransactionObserver_UNSTABLE(({ snapshot }) => {
    const get = atom => snapshot.getLoadable(atom).contents;
...
export function useField(props) {
  ...
  const oldOtherParamsRef = useRef(null);
  const oldValueRef = useRef(null);
  const oldTouchedRef = useRef(false);
...
export function useFieldWatch(props) {
...
export function useFieldArrayColumnWatch(props) {
...
export function useIsDirty(options) {
  const formId = useContext(FormIdContext);
...
export function useFormValues(params) {
...
export function useSetFormProps(params) {
  const { formId: overrideFormId } = params ?? {};
  const defaultFormId = useContext(FormIdContext);
  const formId = overrideFormId ?? defaultFormId;
...
export function useFormValuesAndExtraInfos(params) {
  const { formId: overrideFormId } = params ?? {};
  const defaultFormId = useContext(FormIdContext);
  const formId = overrideFormId ?? defaultFormId;
...
export function useInitialValues(params) {
  const { formId: overrideFormId } = params ?? {};
  const defaultFormId = useContext(FormIdContext);
  const formId = overrideFormId ?? defaultFormId;
...
export function useFormContext(params) {
    ...
    ({ snapshot }) =>
      key => {
        ...
        if (key.type === 'field') {
          ...
          const initialValuesAtom = get(formInitialValuesAtom(formId));
  ...
  const updateValues = useRecoilTransaction_UNSTABLE(
    ({ set, get, reset }) =>
      (values, settings, extraInfos) => {
        resetDataAtoms(reset);
        const existingVal = get(formInitialValuesAtom(formId));
        const newValues = values ?? existingVal.values;
        const newExtraInfos = extraInfos ?? existingVal.extraInfos;
  ...
  const updateDNDValues = useCallback(
    (values, extraInfos) => {
      updateValues(values, undefined, extraInfos);
    },
    [updateValues]
  );
...
export function useFieldArray(props) {
  ...
  const formId = useContext(FormIdContext);
  const initialValues = useRecoilValue(formInitialValuesAtom(formId));
  const prevFieldArrayValue = useRef(null);
  ...
  const getFieldArrayValueByRowId = useRecoilCallback(
    ({ snapshot }) =>
      rowId => {
        const get = snapshotToGet(snapshot);
        return getFieldArrayDataAndExtraInfoByRowId(
          formId,
  ...
  const initializeFieldArrayValue = useRecoilTransaction_UNSTABLE(
    ({ set, get, reset }) =>
      () => {
        const initialValues = get(formInitialValuesAtom(formId));
        console.log(initialValues, 'initializeFieldArrayValue')
  ...
  const resetFieldArray = useRecoilTransaction_UNSTABLE(
    ({ reset, get }) =>
      name => {
        ...
        const fieldArrayAtomValue = value;
  ...
  useEffect(() => {
    console.log(fieldArrayProps.initVer, initialValues.version)
  ...
  return {
    ...
    getFieldArrayValue,
    setFieldArrayValue,
    getFieldArrayValueByRowId,
    error: fieldArrayProps?.error,
  };
}

const getFormValues = (formId, get) => {
  const initialValues = get(formInitialValuesAtom(formId));
  const values =
...
function getValidateAllFieldsFn(props) {
  ...
  return (values, extraInfos) => {
    ...
    for (const fieldAtomInfo of Object.values(
      ...
      const formFieldData = get(fieldAtom);
...
export function useForm(props) {
  ...
  const getValidateFnFromAtom = useRecoilCallback(({ snapshot }) => () => {
    const validationFn = snapshot.getLoadable(
...
function RecoilFormProvider(props) {
...
export function FormProvider(props) {
  const formId = useRef(props?.options?.formId ?? generateFormId());
